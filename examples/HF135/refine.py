...
coarse = solve(D=D, i=theta_i, b=theta_b, itol=5e-2, d_dob_bracket=(-1, 0), verbose=2)

print()
print("----Refined with solve()----")
fine = solve(D=D, i=theta_i, b=theta_b, d_dob_bracket=coarse.d_dob_bracket, verbose=2)
...
from_guess = solve_from_guess(
    D=D, i=theta_i, b=theta_b, o_guess=o_guess, guess=guess, verbose=2
)
...
plt.plot(
    validation.r,
    coarse(validation.r, validation.t),
    label="Starting solution (solve(), higher tolerance)",
)
plt.plot(validation.r, fine(validation.r, validation.t), label="Refined with solve()")
plt.plot(
    validation.r,
    from_guess(validation.r, validation.t),
...
plt.plot(
    validation.r,
    coarse.flux(validation.r, validation.t),
    label="Starting solution (solve(), higher tolerance)",
)
plt.plot(
    validation.r, fine.flux(validation.r, validation.t), label="Refined with solve()"
)
plt.plot(
    validation.r,
    from_guess.flux(validation.r, validation.t),
    label="Refined with solve_from_guess()",
)
plt.xlabel(f"position [{validation.r_unit}]")
